...
async function fetchAdManagerReportYesterday(userId, bearer, networkId) {
    if (!networkId) {
        console.log("Network id not found");
    }
    try {
    ...
    `);
        ...
        // ✅ Step 1: Define YESTERDAY report
        const reportQuery = {
            dimensions: ["DATE", "SITE_NAME", "COUNTRY_NAME"],
            columns: [
                "AD_EXCHANGE_LINE_ITEM_LEVEL_IMPRESSIONS",
            ...
            dateRangeType: "YESTERDAY", // ✅ only yesterday's data
            reportCurrency: "USD",
            timeZoneType: "PUBLISHER",
        };
        ...
        const jobId = jobResult?.rval?.id;
        if (!jobId) throw new Error("❌ Failed to start YESTERDAY report job.");
        ...
        if (status !== "COMPLETED") {
            throw new Error(`❌ YESTERDAY Report failed with status: ${status}`);
        }
        ...
        const downloadUrl = download?.rval;
        if (!downloadUrl)
            throw new Error("❌ Could not get download URL for YESTERDAY");
...
module.exports = { fetchAdManagerReportLast30Days, fetchAdManagerReportToday, fetchAdManagerReportYesterday };